...
expParser = parseRPN . infixToRPN <$> takeTill (== ';') <?> "espresion"
...
stmtParser =
  Print
    ...
    <|> While
    <$> (skipSpace *> string "while" *> expParser <* endParser <?> "while")
    <*> stmtParser
    <|> If
    <$> (skipSpace *> string "if" *> expParser <* endParser <?> "while")
    <*> stmtParser
    <|> Seq
    <$> (   skipSpace
        *>  string "do"
...
shuntOp (out, op : ops) x
  | op ->- x || (op -=- x && isLeftAssociative x) = shuntOp (op : out, ops) x
...
parseFun fs x
  | isOperator x && length fs > 1
  = Right $ parseOp fs x
  | inClass ['a' .. 'z'] $ T.index x 0
  = parseOnly (V <$> varParser) x >>= return . (: fs)
  | otherwise
  = case parseOnly double x of
    Right n   -> Right $ C (Just n) : fs
    Left  msg -> Left msg
