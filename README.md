...
* if: `if <expr1> do <expr2> else <expr3>` (`else` is optional). Value of "if" is `<expr2>` if `<expr1>` is a `true`, non-zero number, non-empty list or a non-empty string, otherwise `<expr3>` or `null` if no `else` is specified.
